...
type
  ...
  TfMain = class(TForm)
    ...
    procedure backToASetClick(Sender: TObject);
    ...
    procedure backToASClick(Sender: TObject);
    procedure deleteActivityButtonClick(Sender: TObject);
    procedure DeleteActivityFromDB(const ActivityID: string);
    procedure DeleteActivityReservationsFromDB(const ActivityID: string);
    procedure DeleteActivityEverywhere(const ActivityID: string);
  private
...
begin
  if Trim(SelectedActivityID) = '' then
  begin
    ShowMessage('Aktivnost nije izabrana.');
    Exit;
  end;

  if MessageDlg('Obrišite ovu aktivnost?', TMsgDlgType.mtConfirmation,
    [TMsgDlgBtn.mbYes, TMsgDlgBtn.mbNo], 0) <> mrYes then
    Exit;

  DeleteActivityEverywhere(SelectedActivityID);
...
procedure TfMain.backToASetClick(Sender: TObject);
begin
tcMain.ActiveTab := tiAdminSettings;
end;
...
procedure TfMain.backToASClick(Sender: TObject);
begin
tcMain.ActiveTab := tiAdminSettings;
end;
...
begin
  ...
  begin
    ShowMessage('Oprema nije izabrana.');
    Exit;
  end;

  if not TryStrToInt(Trim(reserveQtyEdit.Text), Qty) then
  begin
    ShowMessage('Upišite validnu količinu.');
    Exit;
  end;
...
begin
  ...
  begin
    ShowMessage('Izaberite datum.');
    Exit;
  end;

  if SelectedTimeSlot = '' then
  begin
    ShowMessage('Izaberite termin.');
    Exit;
  end;

  if SelectedHall = '' then
  begin
    ShowMessage('Izaberite salu.');
    Exit;
  end;
...
begin
  ...
  if Name = '' then
  begin
    ShowMessage('Upišite ime opreme.');
    Exit;
  end;
  ...
  if Qty < 0 then
  begin
    ShowMessage('Količina ne sme biti negativna.');
    Exit;
  end;
...
begin
  ...
  begin
    ShowMessage('Aktivnost nije pronadjena.');
    Exit;
  end;
  ...
  adminViewActivityDate.Text := 'Date: ' + FormatDateTime('dd.mm.yyyy', D);
  adminViewActivityTime.Text := 'Timeslot: ' + E.TimeSlot;
  adminViewActivityHall.Text := 'Hall: ' + E.Hall;
  adminViewActivityTickets.Text := 'Free spots: ' + IntToStr(E.Tickets);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          equipmentQuantity.Text := 'Available: ' + IntToStr(E2.Quantity);
          equipmentPrice.Text := 'Price: ' + FloatToStr(E2.Price);
      ...
      ShowMessage('Oprema uspešno rezervisana!');
      Exit;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      Close;
      ShowMessage('Već ste rezervisali ovu aktivnost.');
      Exit;
    end;
...
var
  ...
  L: TLabel;
begin
  R := TRectangle.Create(reservedTerminsBox.Content);
  R.Parent := reservedTerminsBox.Content;
  R.Height := 70;
  R.Align := TAlignLayout.Top;
  R.Fill.Color := TAlphaColorRec.DeepSkyBlue;
  R.Stroke.Kind := TBrushKind.Solid;
...
begin
  ...
  reservationDate.Text := 'Datum: ' + FormatDateTime('dd.mm.yyyy', E.EventDate);
  hallName.Text := 'Sala: ' + E.Hall;
  reservationTime.Text := 'Vreme: ' + E.TimeSlot;
...
begin
  ...
  begin
    ...
    SQL.Clear;
    SQL.Text :=
      'DELETE FROM RezervacijaAktivnosti R ' +
      'WHERE R.ID_kor = :kor AND R.ID_aktivnost = :aktivnost ' +
      'AND R.datum_rezervacije = (' +
      '  SELECT MIN(R2.datum_rezervacije) ' +
      '  FROM RezervacijaAktivnosti R2 ' +
      '  WHERE R2.ID_kor = :kor2 AND R2.ID_aktivnost = :aktivnost2' +
      ')';

    ParamByName('kor').AsString := CurrentUserID;
    ParamByName('aktivnost').AsString := ActivityID;
    ParamByName('kor2').AsString := CurrentUserID;
    ParamByName('aktivnost2').AsString := ActivityID;
    ExecSQL;
  end;
end;
...
procedure TfMain.DeleteActivityReservationsFromDB(const ActivityID: string);
begin
  with DataModule2.RezAktivnostiQuery do
  begin
    Close;
    SQL.Clear;
    SQL.Text := 'DELETE FROM RezervacijaAktivnosti WHERE ID_aktivnost = :id';
    ParamByName('id').AsString := ActivityID;
    ExecSQL;
  end;
end;
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Text := 'DELETE FROM Aktivnost WHERE id_aktivnost = :id';
    ParamByName('id').AsString := ActivityID;
    ExecSQL;
  end;
end;

procedure TfMain.DeleteActivityEverywhere(const ActivityID: string);
begin
  if Trim(ActivityID) = '' then
    Exit;

  DeleteActivityReservationsFromDB(ActivityID);
