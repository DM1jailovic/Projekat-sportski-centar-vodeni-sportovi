...
var
  I: Integer;
begin
  // Remove children safely without calling DisposeOf immediately
  for I := AParent.ChildrenCount - 1 downto 0 do
    AParent.Children[I].Parent := nil;  // detach from layout
end;
...
with DataModule2 do
begin
userName.Text := KorisnikTable['Ime'];
              nameSB.Text := KorisnikTable['Ime'];;
end;
...
begin
  ...
  with DataModule2.ZaposleniTable do
  begin
    if not Locate('EMAIL', email, []) then
    begin
    ...
    if (Length(StoredHash) = 64) and (StoredSalt <> '') then
    begin
      if VerifyPassword(pass, StoredSalt, StoredHash) then
      begin
        CurrentUserRole := urAdmin;
        CurrentUserID := AdminID;
        userName.Text := AdminIme;
        nameSB.Text := AdminIme;
      ...
      begin
        ...
        StoredHash := HashPassword(pass, StoredSalt);
        ...
        begin
          ...
          SQL.Text :=
            'UPDATE Zaposleni ' +
            'SET PasswordHash = :hash, PasswordSalt = :salt, Password = :oldpass ' +
            'WHERE ID_zap = :id';

          ParamByName('hash').DataType := ftString;
          ParamByName('hash').Size := 64;
          ParamByName('salt').DataType := ftString;
          ParamByName('salt').Size := 32;
          ParamByName('oldpass').DataType := ftString;
          ParamByName('oldpass').Size := 255;
          ParamByName('id').DataType := ftString;
          ParamByName('id').Size := 50;

          ParamByName('hash').AsString := StoredHash;
          ParamByName('salt').AsString := StoredSalt;
          ParamByName('oldpass').AsString := '';
...
begin
  ...
  // Only reload events if the layout exists and is visible
  if Assigned(eventLayout) and eventLayout.Visible then
    LoadEventsForDate;

  // Optional: navigate to the tab if safe
...
begin
  ...
  with DataModule2.KorisnikTable do
  begin
    if not Locate('EMAIL', email, []) then
    begin
      wrongCred.Visible := True;
    ...
    // Only trust hash if it looks valid
    if (Length(StoredHash) = 64) and (StoredSalt <> '') then
    begin
      if VerifyPassword(pass, StoredSalt, StoredHash) then
      begin
        ...
        userName.Text := UserIme;
        nameSB.Text := UserIme;
        tcMain.ActiveTab := tiHome;
        Exit;
      end;

      // If hash exists but fails, still allow fallback to old plain password
      // only if old plain password is still present
      if (StoredPlain <> '') and (StoredPlain = pass) then
      begin
        StoredSalt := GenerateSalt;
        StoredHash := HashPassword(pass, StoredSalt);
    ...
    // Old plain-text account or broken migration with no valid hash
    if (StoredPlain <> '') and (StoredPlain = pass) then
    begin
      StoredSalt := GenerateSalt;
      StoredHash := HashPassword(pass, StoredSalt);
...
begin
  ...
  R.Fill.Color := TAlphaColorRec.Lightgray;
...
begin
  Result := IntToStr(Random(9000) + 1000); // 4-digit ID
end;
...
begin
  ...
  E.Hall := SelectedHall;
  E.ActivityType := 'General'; // or from a field if you add one
...
begin
  ...
  begin
    ...
    begin
      ...
      // 2. reduce stock in equipment table
      with DataModule2.OpremaQuery do
      begin
        Close;
        SQL.Clear;
      ...
      // 3. reload both grids and detail data
      LoadEquipmentFromDB;

      // 4. reopen selected equipment details to show fresh quantity
...
begin
  ...
  ItemLayout.Align := TAlignLayout.Client;
...
begin
  ...
  R.Height := 70;
  R.Align := TAlignLayout.Top;
  R.Fill.Color := TAlphaColorRec.Lightgreen;
  R.Stroke.Kind := TBrushKind.Solid;
  R.Stroke.Color := TAlphaColorRec.Black;
  R.Margins.Top := 4;
  R.Margins.Bottom := 4;
...
begin
  ...
  // reload activities for the same date so free places update
